...
{
	return a > b; //ÉýÐòÅÅÁÐ£¬Èç¹û¸ÄÎªreturn a>b£¬ÔòÎª½µÐò 
}
...
int gen_chain_word(char* words[], int len, char* result[], char head, char tail, bool enable_loop)
{
	int re = 0;
	Core *c = new Core();
	re = c->a->gen_chain_word(words, len, result, head, tail, enable_loop);
	return re;
}
...
int gen_chain_char(char* words[], int len, char* result[], char head, char tail, bool enable_loop)
{
	int re = 0;
	Core *c = new Core();
	re = c->a->gen_chain_char(words, len, result, head, tail, enable_loop);
	return re;
}